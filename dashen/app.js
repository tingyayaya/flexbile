...
schedule.scheduleJob(rule, function(){
  ...
  mydataSkt46.getMyData().then(function(data){
    if(data){
      const fileName = 'HY-EMP_'+getDate();
      const ROOT_PATH = path.resolve(__dirname, 'file/HY-EMP', fileName+'.txt');
  ...
  mydataSkt52.getMyData().then(function(data){
    if(data){
      const fileName = 'HY-PRE_'+getDate()
      const ROOT_PATH = path.resolve(__dirname, 'file/HY-PRE', fileName+'.txt');
      createDocument(ROOT_PATH, data)
    }
  }, function(error){
    var log_info = log4js.getLogger('info');
